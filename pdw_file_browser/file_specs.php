...
switch($type){
	case 'image':
        ...
        $file_size = $file_size < 1024  ? $file_size. ' '.translate_pdw('bytes') : $file_size < 1048576 ? number_format($file_size / 1024, 2, $dec_seperator, $thousands_separator) . ' '.translate_pdw('kB') : number_format($file_size / 1048576, 2, $dec_seperator, $thousands_separator) . ' '.translate_pdw('MB');
		...
		printf('<div class="filemodified"><span>%s:&nbsp;</span>%s</div>',translate_pdw('Modified on'), $file_modified);
		printf('<div class="filesize"><span>%s:&nbsp;</span>%s</div>',translate_pdw('Size'), $file_size);
		printf('<div class="filedim"><span>%s:&nbsp;</span>%s x %s</div>',translate_pdw('Dimensions'), $image_info[0], $image_info[1]);
		break;
	case 'file':
		$filename = DOCUMENTROOT.$uploadpath;
		$file_modified = date($datetimeFormat, filemtime($filename));
	...
	default: // folder
		...
		printf('<div class="filetype">%s</div>', translate_pdw('Directory'));
		printf('<div class="filemodified"><span>%s:&nbsp;</span>%s</div>',translate_pdw('Modified on'), $folder_modified);
		break;
}
