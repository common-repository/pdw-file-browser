...
{
    echo 'error||' . translate_pdw("Action not allowed!");
    exit();
}
...
switch($action){
			...
			if(count($files) && is_dir($folder)){
				...
				foreach($files as $key => $value){
					...
					if(($file = checkpath($value, $uploadpath)) && (DOCUMENTROOT.$file != $folder)) { //Check if folder/file is valid
						...
						echo 'error||'.translate_pdw("The folder path was tampered with!");	
						break;	
					}
				}
				...
				if($result){
					...
					foreach($files as $key => $value){
						...
						if($result){
							echo 'success||'.sprintf(translate_pdw("%d file(s) successfully removed!"), "1");
						} else {
							echo 'error||'.translate_pdw("The file or folder path was tampered with!");	
						}
					}
				} else {
					echo 'error||'.translate_pdw("The file or folder path was tampered with!");	
				...
				echo 'success||'.sprintf(translate_pdw("%d file(s) successfully removed!"), count($files));
	...
	case "copy_paste":
		...
		if($folder = checkpath($folder, $uploadpath)){
			...
			if(count($files) && is_dir($folder)){
				...
				if($result){
					echo 'success||'.translate_pdw("The files where successfully copied!");
				} else {
					echo 'error||'.translate_pdw("The file or folder path was tampered with!");	
				}
	...
	case "rename":
		...
		if($folderpath = checkpath($folderpath, $uploadpath)){
			...
			if($new_filename != "" && $old_filename != "" && is_dir($folder)){
				...
				if($type == 'folder'){
					if(is_dir($dest)){
						echo "error||" . translate_pdw("Directory already exists!");
						break;
					} 			
				} else {
					if(is_file($dest)){
						echo "error||" . translate_pdw("File already exists!");
						break;
					}
				}
					
				if(@rename($source, $dest)){
					echo "success||" . translate_pdw("Name successfully changed!");
				} else {
					echo "error||" . translate_pdw("Rename failed!");	
				}
	...
	case "delete":
		...
		if($result){
			...
			echo 'error||'.translate_pdw("Deleting file failed!");	
		}
	...
	case "create_folder":
		...
		if($foldername == ""){
			echo 'error||' . translate_pdw("Creating new folder failed!");
			break;
		}
		...
		if (@mkdir(DOCUMENTROOT.$folderpath.$foldername)){
			echo 'success||' . translate_pdw("A new folder was created!");
		} else {
			echo 'error||' . translate_pdw("Creating the new folder failed!");
		}
		
		break;
		
	case "settings":
        echo translate_pdw('Settings saved!');
        break;
}
?>